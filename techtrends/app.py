...
# variable to keep track of total connection to database
...
# Function to get a total post Count in the db
def get_post_count():
    connection = get_db_connection()
    post_count = connection.execute('SELECT COUNT(id) as count FROM posts').fetchone()  
    connection.close()
